...
<div class="common-form">
    <table width="100%" class="table_form contentWrap">
        ...
        <tr>
            ...
            <td>
                <table>
                    <tr>
                        <td><span id="a_tip"
                                  class="onShow">1.如果为非功能性菜单,则无需填写!2.功能性菜单分为MCA(模块->控制器->方法)和URL两种,任选其一填写即可</span></td>
                    </tr>
                    <tr>
                        <td>
                            模块:<input type="text" name="Menu[m]" id="menu_m" value="<?= $model->m; ?>"
                                      class="input-text" style="width:60px;">
                            控制器:<input type="text" name="Menu[c]" id="menu_a" value="<?= $model->c; ?>"
                                       class="input-text" style="width:60px;">
                            方法:<input type="text" name="Menu[a]" id="menu_c" value="<?= $model->a; ?>"
                                      class="input-text" style="width:60px;">
                            /
                        </td>
                    </tr>
                    <tr>
                        <td>URL:<input type="text" name="Menu[url]" id="menu_url" value="<?= $model->url; ?>"
        ...
        <tr>
            <th width="120">描述：</th>
            <td><textarea rows="2" cols="30" name="Menu[description]"
                          class="input-text"><?= $model->description; ?></textarea></td>
        </tr>
        <tr>
            <th width="120">是否显示为菜单：</th>
            <td><input type="radio" name="Menu[display]" value="<?= MENU_DISPLAY_YES ?>"
                       <?php if ($model->display == MENU_DISPLAY_YES): ?>checked<?php endif; ?>>是
                <input type="radio" name="Menu[display]" value="<?= MENU_DISPLAY_NO ?>"
                       <?php if ($model->display == MENU_DISPLAY_NO): ?>checked<?php endif; ?>> 否
            </td>
        </tr>
    </table>
    <div class="bk15"></div>
    ...
    <script type="text/javascript">
        $(function () {
            $.formValidator.initConfig({
                formid: "menuForm", autotip: true, onerror: function (msg, obj) {
            ...
            $("#menu_name").formValidator({
                onshow: "请输入菜单英文名称",
                onfocus: "请输入菜单英文名称",
                oncorrect: "输入正确"
            }).inputValidator({min: 1, onerror: "请输入菜单英文名称"});
            ...
            $("#menu_url").formValidator({empty: true}).regexValidator({
                regExp: "url",
                dataType: "enum",
                onerror: "Url格式有误"
            });
